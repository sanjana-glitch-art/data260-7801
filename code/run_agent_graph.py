...
class TrialCreate(BaseModel):
    """Values accepted during creation."""
...
trials: list[ClinicalTrial] = [
    ClinicalTrial(
        ...
        trial_description=(
            "This clinical trial examines "
            "how sleep quality affects "
            "university students."
        ),
...
def cleaned_required_value(
    ...
    """Strip and validate a required value."""
...
def find_trial(
    ...
    """Return a trial or raise HTTP 404."""
...
def matching_trials(
    ...
    return [
        ...
        if (
            query
            in trial.trial_title.casefold()
            or query
            in trial.sponsor_name.casefold()
        )
    ]


@app.get("/trials")
async def trial_application() -> FileResponse:
    """Serve the existing trial application."""
...
async def create_trial_api(
    ...
    """Create through the JSON API."""

    trial_title = (
        cleaned_required_value(
...
async def create_trial_form(
    trial_title: Annotated[
        str,
        Form()
    ],
    sponsor_name: Annotated[
        str,
        Form()
    ],
    submitter_email: Annotated[
        str,
        Form()
    ],
    trial_description: Annotated[
        str,
        Form()
    ],
    trial_phase: Annotated[
        str,
        Form()
    ]
) -> RedirectResponse:
    """Create through the HTML form."""
...
async def delete_highest_trial_form(
) -> RedirectResponse:
